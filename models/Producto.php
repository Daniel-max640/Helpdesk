<?php
    class Producto extends Conectar{
        ...
        public function productos_servicios($descripcion){
            ...
            parent::set_names();
            $sql="SELECT p.id_modelo, p.descripcion, m.abreviatura AS medida_descripcion, p.precio 
            FROM tm_modeloportatil p 
            INNER JOIN tm_umedida m ON p.id_medida = m.id_medida
            WHERE p.descripcion LIKE '%$descripcion%'
            UNION
            SELECT s.id_servicio, s.descripcion,m.abreviatura AS medida_descripcion, s.precio 
            FROM tm_servicio s 
